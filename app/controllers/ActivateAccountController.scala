...
class ActivateAccountController @Inject() (
  ...
  def send(email: String) = silhouette.UnsecuredAction.async { implicit request: Request[AnyContent] =>
    ...
    val result = Redirect(routes.SignInController.index()).flashing("info" -> Messages("activation.email.sent", decodedEmail))
      ...
      case Some(authToken) => userService.retrieve(authToken.userID).flatMap {
        case Some(user) if user.loginInfo.providerID == CredentialsProvider.ID =>
          userService.save(user.copy(activated = true)).map { _ =>
            Redirect(routes.SignInController.index()).flashing("success" -> Messages("account.activated"))
          }
        case _ => Future.successful(Redirect(routes.SignInController.index()).flashing("error" -> Messages("invalid.activation.link")))
      }
      case None => Future.successful(Redirect(routes.SignInController.index()).flashing("error" -> Messages("invalid.activation.link")))
    }
  }
}
