...
import infra.{ CacheAuthenticatorRepositoryImpl, RedisCacheLayer }
...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  def configure() {
    ...
    bind[DelegableAuthInfoDAO[PasswordInfo]].toInstance(new AuthInfoDao)
    bind[DelegableAuthInfoDAO[OAuth1Info]].toInstance(new InMemoryAuthInfoDAO[OAuth1Info])
    bind[DelegableAuthInfoDAO[OAuth2Info]].toInstance(new InMemoryAuthInfoDAO[OAuth2Info])
    bind[DelegableAuthInfoDAO[OpenIDInfo]].toInstance(new InMemoryAuthInfoDAO[OpenIDInfo])

    bind[AuthenticatorRepository[CookieAuthenticator]].to[CacheAuthenticatorRepositoryImpl]
  }
  ...
  def provideAuthenticatorService(
    ...
    cacheLayer: RedisCacheLayer,
    clock: Clock): AuthenticatorService[CookieAuthenticator] = {

    val config = configuration.underlying.as[CookieAuthenticatorSettings]("silhouette.authenticator")
    val authenticatorEncoder = new CrypterAuthenticatorEncoder(crypter)

    val authenticatorRepository = new CacheAuthenticatorRepository[CookieAuthenticator](cacheLayer)
    new CookieAuthenticatorService(config, Some(authenticatorRepository), signer, cookieHeaderEncoding, authenticatorEncoder, fingerprintGenerator, idGenerator, clock)
  }
